...
array<int, 3> getDifference(AbstractBoard& newBoard, AbstractBoard& original) {
    //assumes only a single cell difference. "returns" the tabu triplet
    SudokuBoard originalBoard = original.getBoard();
    SudokuBoard newSudokuBoard = newBoard.getBoard();
    for(int row = 0; row < 9; row++) {
        for(int col = 0; col < 9; col++){
            

            if(originalBoard.board[row][col] != newSudokuBoard.board[row][col]) {
                array<int, 3> tabu;
                tabu[0] = row;
                tabu[1] = col;
                tabu[2] = originalBoard.board[row][col];
                return tabu;
            }
        }
    }
...
AbstractBoard tabuSearch(AbstractBoard inputBoard, int maxIter, int tabuLength, int candidateLength) {


    AbstractBoard bestBoard = inputBoard;
    globalTabuLength = tabuLength;
    deque<array<int, 3>> tabuQueue; // use deque for random access to check existing tabus

    for(int iter = 0; iter < maxIter; iter++) {
        AbstractBoard candidateList[candidateLength];
        int candidates = 0;
        while(candidates < candidateLength) {
            AbstractBoard newCandidate = bestBoard.sample();
            bool isValid = true;
            for(int i = 0; i < tabuQueue.size(); i++){
                int tabu[3] = {tabuQueue[i][0], tabuQueue[i][1], tabuQueue[i][2]};
                if(newCandidate.getBoard().board[tabu[0]][tabu[1]] == tabu[2]){
                    isValid = false;
                    break;
                }
            }
        ...
        AbstractBoard *nextBest;
        int bestFitness = 0;
        for(AbstractBoard candidate : candidateList) {
        ...
        if(bestFitness > bestBoard.getFitness()) {
