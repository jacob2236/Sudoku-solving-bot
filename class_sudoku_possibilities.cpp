    ...
    public:
        ...
        PossibilitiesBoard(string boardString){ //takes an 81 char string representing the board. '.' represents an empty space
            ...
            for(int row = 0; row < 9; row++) {
                for(int col = 0; col < 9; col++) {
                    if(board[row][col] != 0) {
                        possibilies[row][col].insert(-1); // -1 marks the cell as a given
                    } else {
                        possibilies[row][col] = getPossibilities(row, col);
                    }
                }
            }
        }
        ...
        PossibilitiesBoard mutate() {
            ...
            do {
                randRow = rand() % 9;
                randCol = rand() % 9;
                set<int> cellPossibilities = possibilies[randRow][randCol];
                if(cellPossibilities.count(-1) == 0){
                    ...
                    if(board[randRow][randCol] == 0) {
                        ...
                        return PossibilitiesBoard(newBoard);
                    } else {
            ...
            } while (searching); // repeat if there's a failure
        }
